#!/usr/bin/env bash
# Stop hook: instructs Claude to update CLAUDE.md when code files are modified.
set -euo pipefail
...
MODIFIED=$(echo "$MODIFIED" | sort -u | grep -v '^$' | grep -Ev '(^CLAUDE\.md$|\.(md|txt|rst|adoc|asciidoc)$)' || true)

if [[ -z "$MODIFIED" ]]; then
  exit 0
fi

echo "Code changes detected. Please update CLAUDE.md using the init-claude-md skill."
