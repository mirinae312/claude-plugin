---
description: Generate unit tests for a JavaScript or TypeScript source file using the test framework and libraries already declared in the project.
---

## Goal

Generate unit tests for the target `.js`, `.ts`, `.jsx`, or `.tsx` file or function. Always use the test framework and libraries already declared in the project. Never introduce a new test dependency without asking the user first.
...
- Otherwise, use the most recently edited `.js`/`.ts`/`.jsx`/`.tsx` file.

### 2. Detect the project's test stack

Read `package.json`, `jest.config.*`, `vitest.config.*`, or `.mocharc.*` and identify the framework in use.
...
- Cover for each public function / method:
  ...
  - **Edge cases** — undefined/null, empty collections, boundary values
  - **Error cases** — exceptions thrown, rejected promises
